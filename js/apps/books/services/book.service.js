...
function getBookById(id){
  var books = storageService.load(BOOKS_KEY);
  if(!books) books = booksDB
  var book = books.find(book => book.id === id)
  return Promise.resolve(book);
}


function addReview(book, rev){
  var books = storageService.load(BOOKS_KEY);
  var idx = books.findIndex(currBook => {
...
function getGBooks(val){

  var gBooks 
  
  var prmRes = axios.get(`https://www.googleapis.com/books/v1/volumes?q=${val}`)
  var prmBooks = prmRes.then(res => {
    var books = res.data.items;
    gBooks = books.map(book => {
      var isForSale = (book.saleInfo.saleability === 'FOR_SALE');
        return {
            id: book.id,
            ...
            listPrice: {
              amount: (isForSale)? book.saleInfo.listPrice.amount : utilService.getRandomInt(100,1000),
              currencyCode: (isForSale)? book.saleInfo.listPrice.currencyCode: 'ILS',
              isOnSale: (isForSale)? true : false
            },
            reviews: []
        }
    })
